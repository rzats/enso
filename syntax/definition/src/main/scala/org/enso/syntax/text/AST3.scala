package org.enso.syntax.text.v3
...
//      }
  ...
  case class Node[+H[_], F[_]](unFix: H[Node[F, F]], id: Option[ID] = None)(
    ...
    def map(f: AST => AST): Node[H, F] = Node(ops.map(unFix)(f))
  }
  object Node {
    object implicits extends implicits
    trait implicits {
...
//        extends InvalidOf[T]
    ...
    object Opr {
      ...
      type Mod = AST.Mod // FIXME: Compatibility mode
      val Mod = AST.Mod // FIXME: Compatibility mode
    }
